...
function Counter() {
    ...
    const start = React.useCallback(() => {
        interval.current = setInterval(() => { setCounter(c => c + 1 ) }, 500);
    }, []);
    ...
    React.useEffect(() => { 
        ...
        if (counter == 10){
            stop();  
            var popup = $("#popup").dialog({modal:true});
        }
    }, [counter]);
    ...
    return(
        <div className="text-center">
            ...
            <div id="popup" title="Counter stopped" style={{display:"none"}}>Counter has reached maximum value: {counter}</div>           
        </div>         
    );
}
